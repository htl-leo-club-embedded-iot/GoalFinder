...
bool Settings::Begin(const char *name, bool readOnly, const char *partition_label) {
    ...
    if (!mInitialized) {
        ...
        if (rfs != 0) {
            ...
            Logger::log("Settings", Logger::LogLevel::DEBUG, "Opening preferences file '%s', RO: %c",
                        prefFilePath.c_str(), readOnly ? 'Y' : 'N');
            if (rfs->Exists(mNvsPath) || rfs->MkDir(mNvsPath)) {
                mFile = rfs->Open(prefFilePath, (mReadOnly ? RootFileSystem::FileMode::Read
                                                           : RootFileSystem::FileMode::WriteTruncate));
                rc = (bool)mFile;
                if (!rc)
                    Logger::log("Settings", Logger::LogLevel::ERROR, "Failed opening preferences file '%s'.",
                                prefFilePath.c_str());
                if (rc) {
                    size_t bufferLen = mFile.size() + 1;
                    Logger::log("Settings", Logger::LogLevel::DEBUG, "Opened preferences file '%s', %d B",
                                prefFilePath.c_str(), mFile.size());
                    ...
                    if (mFile.size() > 0) {
                        ...
                        if (!rc)
                            Logger::log("Settings", Logger::LogLevel::ERROR,
                                        "Failed reading preferences from file '%s': %s", prefFilePath.c_str(),
                                        res.c_str());
                    }
                }
                if (mReadOnly && mFile) {
...
size_t Settings::PutChar(const char *key, int8_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_I8,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? 1
               : 0;
}

size_t Settings::PutUChar(const char *key, uint8_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_U8,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? 1
               : 0;
}
...
size_t Settings::PutShort(const char *key, int16_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_I16,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? 2
               : 0;
}
...
size_t Settings::PutUShort(const char *key, uint16_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_U16,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? 2
               : 0;
}
...
size_t Settings::PutInt(const char *key, int32_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_I32,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? 4
               : 0;
}
...
size_t Settings::PutUInt(const char *key, uint32_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_U32,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? 4
               : 0;
}
...
size_t Settings::PutULong64(const char *key, uint64_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_U64,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? 8
               : 0;
}
...
size_t Settings::PutFloat(const char *key, const float_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_FLOAT,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? sizeof(float_t)
               : 0;
}
...
size_t Settings::PutDouble(const char *key, const double_t value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_DOUBLE,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? sizeof(double_t)
               : 0;
}
...
size_t Settings::PutBool(const char *key, const bool value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_BOOL,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? sizeof(bool)
               : 0;
}
...
size_t Settings::PutString(const char *key, const char *value) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_STR,
                                       [value](JsonVariant &valueNode) { valueNode.set(value); })
               ? strlen(value)
               : 0;
}
...
size_t Settings::PutBytes(const char *key, const void *value, size_t len) {
    return GetInstanceData()->PutValue(key, SettingsType::PT_BLOB,
                                       [value, len](JsonVariant &valueNode) {
