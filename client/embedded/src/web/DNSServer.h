...
public:
     ...
     * Starts the UDP socket on port 53 and sets the IP address to return for
        * `goalfinder.local` and normalized `<device-name>.local` A-record queries.
     */
    void Begin(const IPAddress& resolvedIP, const String& deviceName);
...
private:
    ...
    /** Primary hostname this server resolves (lower-case). */
    static const char* HOSTNAME;

    WiFiUDP   _udp;
    IPAddress _resolvedIP;
    /** Optional normalized device-name alias hostname (e.g. goalfinder-01.local). */
    String    _deviceAliasHost;
