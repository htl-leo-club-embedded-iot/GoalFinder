...
size_t Settings::PutBool(const char *key, const bool value) {
    Logger::Log("Settings", Logger::LogLevel::DEBUG, "Dummy Settings: PutBool(\"%s\", %s)", key,
                value ? "true" : "false");
    return 0;
}
...
bool Settings::GetBool(const char *key, const bool defaultValue) {
    Logger::Log("Settings", Logger::LogLevel::DEBUG, "Dummy Settings: GetBool(\"%s\", %s)", key,
...
String Settings::GetString(const char *key, const String defaultValue) {
    Logger::Log("Settings", Logger::LogLevel::DEBUG, "Dummy Settings: GetString(\"%s\", %s)", key,
                defaultValue.c_str());
    return "no data";
}
